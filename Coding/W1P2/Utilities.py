...
def create_sim(integrator,start_time,TBP):
    """
    ...
    JD_str = 'JD'+str(start_time)

    if TBP:
        sim.add('Sun',date = JD_str)
        sim.add('Mercury',date = JD_str)
        sim.add('Venus',date = JD_str)
        sim.add("Earth",date = JD_str)
        sim.add('Moon',date = JD_str)
        sim.add("Mars",date = JD_str)
        sim.add("Jupiter",date = JD_str)
        sim.add("Saturn",date = JD_str)
        sim.add("Uranus",date = JD_str)
        sim.add("Neptune",date = JD_str)
    else:
        primary = rebound.Particle(m=1.)
        sim.add(primary)
    sim.integrator = integrator 

    sim.t = (2*np.pi)/start_time
...
def plot_elements(sim,times,kepler,cartesian,body):
    ...
    plt.savefig(f"Coding/W1P2/{re.sub(r"[^\w\-_\. ]", "_", body)}_Cartesian_state_evolution_of.pdf",dpi=300)
    # plt.show()
    ...
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(f"Coding/W1P2/{re.sub(r"[^\w\-_\. ]", "_", body)}_Keplerian_elements.pdf",dpi=300)
    # plt.show()
    ...
    # radius_sun = consts.sun['radius']*1000/const.au
    ...
    plt.legend()
    plt.savefig(f"Coding/W1P2/{re.sub(r"[^\w\-_\. ]", "_", body)}_3D_trajectory.pdf",dpi=300)
    # plt.show()
...
def plot_difference(states, x_scale, body, c=False):
    if c:
        ...
        states_scaled[:,3:] = states[:,3:]*const.au/(1000*const.day)
        ...
        fig.tight_layout(rect=[0, 0, 1, 0.96])
        plt.savefig(f"Coding/W1P2/{re.sub(r"[^\w\-_\. ]", "_", body)}_Cartesian_state_difference.pdf",dpi=300)
        # plt.show()
        ...
        plt.savefig(f"Coding/W1P2/{re.sub(r"[^\w\-_\. ]", "_", body)}_Cartesian_state_difference_norm.pdf",dpi=300)
        # plt.show()
        ...
        plt.savefig(f"Coding/W1P2/{re.sub(r"[^\w\-_\. ]", "_", body)}_Keplerian_state_difference_norm.pdf",dpi=300)
        # plt.show()   

def plot_NGA_acc(F_NGA,x_scale,body,RTN=False):
    ...
    if RTN:
        ...
        fig.suptitle(f"NGA components of {body} in Inertial")
        additional = 'Inertial'
    ...
    axes[3].plot(x_scale, np.linalg.norm(states_scaled, axis=1))
    axes[3].scatter(x_scale[0], states_scaled[0, 1], label='start')
    axes[3].scatter(x_scale[-1], states_scaled[-1, 1], label='stop')
    axes[3].grid(True, which='both')
    axes[3].set_xscale("log")
    axes[3].legend()
    axes[3].set_xlabel("Heliocentric distance [AU]")
    axes[3].set_ylabel(r"$||a||$ $[m/s^2]$")
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    fig.tight_layout(rect=[0, 0, 1, 0.96])

    plt.savefig(f"Coding/W1P2/{re.sub(r"[^\w\-_\. ]", "_", body)}_NGA_components_AU_{additional}.pdf",dpi=300)
    # plt.show()   

def plot_NGA_acc_MJD(F_NGA,x_scale,body,MJD_perih,RTN=False):
    fig, axes = plt.subplots(1, 4, figsize=(15, 8))
    axes = axes.flatten()
    if RTN:
        fig.suptitle(f"NGA components of {body} in RTN/RWS")
    ...
    plt.savefig(f"Coding/W1P2/{re.sub(r"[^\w\-_\. ]", "_", body)}_NGA_components_MJD_{additional}.pdf",dpi=300)
    # plt.show()   