...
# data
with open("Coding/W4/data_2023 H2/info_trace_1000_1.pkl", "rb") as f:
    info = pickle.load(f)
with open("Coding/W4/data_2023 H2/data_trace_1000_1.pkl", "rb") as f:
    data = pickle.load(f)

body = info['Info'].get('Body')

rel_dir = os.path.dirname(os.path.abspath(__file__))
directory_name = os.path.join(rel_dir, f"data_{body}")
...
def compute_family(Trajectory_data, dict, Time_array, perihelion_time):
    for key in Trajectory_data.keys():
        ...
        dict["Clone_Divergence"][key] = (Trajectory_data[key][:, 0:3] - reference) 
...
def plot_divergence_timeline(NGA, NGAf, sample_dis,info_dict):
    ...
    plt.tight_layout()
    plt.savefig(f"Coding/W4/NGAtoNGAFnom/stats_timeline_{safe_name}.pdf",dpi=300)
    plt.close()
...
def plot_divergence_timeline_difference(NGA, NGAf, sample_dis,info_dict):
    ...
    plt.savefig(f"Coding/W4/NGAtoNGAFnom/stats_timeline_difference_{safe_name}.pdf",dpi=300)
    plt.close()
...
def plot_histogram(hist_data, hist_data_f, simulation_info):
    ...
    if np.all(np.isnan(values)):
        ...
        plt.savefig(f"Coding/W4/NGAtoNGAFnom/Histogram_1AU_{safe_name}.pdf", dpi=300)
        plt.close()


def plot_histogram_60D(hist_data, hist_data_f, simulation_info):
    body = simulation_info['Body']
    safe_name = re.sub(r'[^\w\-_\." "]', '_', body)
    ...
    if np.all(np.isnan(values)):
        ...
        plt.tight_layout(rect=[0, 0.03, 1, 0.95])
        plt.savefig(f"Coding/W4/NGAtoNGAFnom/Histogram_60D_{safe_name}.pdf", dpi=300)
        plt.close()
...
plot_histogram_60D(NGA_family["Clone_Divergence_Norm_60D"],NGAf_family["Clone_Divergence_Norm_60D"],info_dict)
