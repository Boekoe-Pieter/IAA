...
def compute_difference(arc,models,data):
    diff = {}
    PG = data[arc][models[0]]
    for model in models[1:]:
        diff[model] = data[arc][model] - PG
...
def plot_difference(arcs, difference, x_scale, body, title, saver, suptitle):
    ...
    x_zoom_max = 1.5*x_min
    # ax.set_xlim(x_zoom_min, x_zoom_max)
    ax.set_yscale('log')

    fig.suptitle(f"{title} {body}", fontweight="bold")
    fig.text(0.5, 0.92, suptitle, ha="center", fontsize=12)

    plt.savefig(
        f"Coding/W2P1/{re.sub(r'[^\w\-_\. ]', '_', body)}_Cartesian_{saver}.pdf", 
        dpi=300
    )
    # plt.show()
    plt.close(fig)
...
def plot_NGA_acc_MJD(arcs, F_NGA, time, body, MJD_perih, suptitle):
    ...
    labels = [r'$a_R$ $[m/s^2]$', r'$a_T$ $[m/s^2]$', r'$a_N$ $[m/s^2]$']
    fig.suptitle(f"NGA components of {body} in RTN/RWS")
    fig.text(0.5, 0.92, suptitle, ha="center")
    ...
    for j_arc, arc in enumerate(arcs):
        ...
        for k_model, model in enumerate(models[1:]): 
            ...
            for i in range(3):
                ...
                #     else:
                #         margin = 5
                #     axes[i].set_xlim(-tau_max - margin, tau_max + margin)
            ...
            #     else:
            #         margin = 5
            #     axes[3].set_xlim(-tau_max - margin, tau_max + margin)
    ...
    plt.savefig(f"Coding/W2P1/{safe_name}_NGA_components_MJD_RTN.pdf", dpi=300)
    # plt.show()
    plt.close(fig)
...
def plot_gr_AU(arcs, g_r, x_scale, body, MJD_perih, suptitle):
    ...
    safe_name = re.sub(r'[^\w\-_\. ]', '_', body)
    plt.savefig(f"Coding/W2P1/{safe_name}_gr_AU.pdf", dpi=300)
    # plt.show()
    plt.close(fig)
...
def plot_gr_MJD(arcs, g_r, time, body, MJD_perih, suptitle):
    ...
    safe_name = re.sub(r'[^\w\-_\. ]', '_', body)
    plt.savefig(f"Coding/W2P1/{safe_name}_gr_MJD.pdf", dpi=300)
    # plt.show()
    plt.close(fig)
