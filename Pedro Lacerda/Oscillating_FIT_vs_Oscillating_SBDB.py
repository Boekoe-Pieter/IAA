...
for comet in comets:
    ...
    start_time = total_data["objects"][body]["observations"].get("earliest")
    ...
    times = np.arange(start_time_MJD, Tp_mjd+timestep, timestep)
    ...
    def diff_orbit_fits(data):
        ...
        fig.suptitle(f'{comet} - Difference between oscillating fit and JPL fitted')
        plt.tight_layout(rect=[0, 0, 1, 0.97])
        plt.savefig(f"Pedro Lacerda/{comet}/Oscillating_Fit_vs_SBDB.pdf",dpi=300)
        plt.show()
